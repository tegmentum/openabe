//! AC17 CP-ABE with White-Box Traitor Tracing
//!
//! This module extends the AC17 CP-ABE scheme with white-box traitor
...
//! # Approach
//!
//! Same as Waters traceable: embeds H(user_id)^τ in each key for tracing.
//!
//! # Efficiency
//!
//! AC17 is more efficient than Waters '11, with constant-size key components.
//! The tracing overhead is minimal (one G1 element per key).

use crate::error::AbeError;
use crate::lsss::PolicyNode;
use crate::tracing::{UserId, TraceResult, TracingMetadata};
use crate::schemes::ac17::{Mpk, Msk, SecretKey, FullCiphertext};
...
/// Traceable Master Public Key for AC17
#[derive(Clone, Debug)]
pub struct Ac17TraceableMpk {
    /// Base AC17 master public key
    pub base: Mpk,
...
/// Traceable Master Secret Key for AC17
#[derive(Clone, Debug)]
pub struct Ac17TraceableMsk {
    /// Base AC17 master secret key
    pub base: Msk,
...
/// Tracing Key for AC17
#[derive(Clone, Debug)]
pub struct Ac17TracingKey {
    /// Tracing secret τ
    pub trace_sk: Fr,
...
/// Proof of identity embedded in AC17 secret key
#[derive(Clone, Debug)]
pub struct Ac17IdentityProof {
...
/// Traceable Secret Key for AC17
#[derive(Clone, Debug)]
pub struct Ac17TraceableSecretKey {
    ...
    /// Identity proof
    pub identity_proof: Ac17IdentityProof,
}

impl Ac17TraceableSecretKey {
    /// Get the user ID
...
/// Setup: Generate traceable master keys and tracing key for AC17
pub fn traceable_setup<R: RngCore + CryptoRng>(rng: &mut R) -> (Ac17TraceableMpk, Ac17TraceableMsk, Ac17TracingKey) {
    let (base_mpk, base_msk) = base_setup(rng);

    let trace_sk = Fr::random(rng);
    let trace_vk = G2::one() * trace_sk;

    let mpk = Ac17TraceableMpk {
        base: base_mpk,
        trace_vk,
    };

    let msk = Ac17TraceableMsk {
        base: base_msk,
        trace_sk,
    };

    let tracing_key = Ac17TracingKey {
        trace_sk,
        trace_vk,
    };
...
pub fn traceable_keygen<R: RngCore + CryptoRng>(
    rng: &mut R,
    mpk: &Ac17TraceableMpk,
    msk: &Ac17TraceableMsk,
    user_id: impl Into<UserId>,
    attributes: &[String],
) -> Result<Ac17TraceableSecretKey, AbeError> {
    ...
    let identity_proof = Ac17IdentityProof {
...
pub fn trace_key(
    _mpk: &Ac17TraceableMpk,
    tracing_key: &Ac17TracingKey,
    leaked_key: &Ac17TraceableSecretKey,
    known_users: &[UserId],
) -> TraceResult {
...
pub fn trace_keys_batch(
    mpk: &Ac17TraceableMpk,
    tracing_key: &Ac17TracingKey,
    leaked_keys: &[Ac17TraceableSecretKey],
    known_users: &[UserId],
...
// ============================================================================
// Encryption/Decryption (delegates to base AC17 scheme)
// ============================================================================
...
mod tests {
    ...
    #[test]
    fn test_ac17_traceable_keygen() {
        let mut rng = thread_rng();
        let (mpk, msk, tracing_key) = traceable_setup(&mut rng);

        let attrs = vec!["admin".to_string(), "level:5".to_string()];
    ...
    #[test]
    fn test_ac17_trace_key_success() {
        let mut rng = thread_rng();
        let (mpk, msk, tracing_key) = traceable_setup(&mut rng);
        ...
        assert!(result.is_traced());
        assert_eq!(result.traitors().unwrap()[0].as_str(), "bob");
    }

    #[test]
    fn test_ac17_trace_unknown_user() {
        let mut rng = thread_rng();
        let (mpk, msk, tracing_key) = traceable_setup(&mut rng);
    ...
    #[test]
    fn test_ac17_verify_key_origin() {
        let mut rng = thread_rng();
        let (mpk, msk, tracing_key) = traceable_setup(&mut rng);
    ...
    #[test]
    fn test_ac17_encrypt_decrypt_traceable() {
        let mut rng = thread_rng();
        ...
        let plaintext = b"AC17 traceable secret";
