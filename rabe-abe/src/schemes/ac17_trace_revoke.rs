//! AC17 Trace-and-Revoke ABE
//!
//! This module combines AC17 CP-ABE with integrated traitor tracing
...
//! - Agrawal-Chase 2017 CP-ABE (base scheme)
...
use crate::schemes::ac17::{Mpk, Msk, SecretKey, Ciphertext, FullCiphertext};
use crate::schemes::ac17::{
    setup as base_setup,
...
/// AC17 Trace-and-Revoke Master Public Key
#[derive(Clone, Debug)]
pub struct Ac17TraceRevokeMpk {
    /// Base AC17 MPK
    pub base: Mpk,
...
/// AC17 Trace-and-Revoke Master Secret Key
#[derive(Clone, Debug)]
pub struct Ac17TraceRevokeMsk {
    /// Base AC17 MSK
    pub base: Msk,
...
#[derive(Clone, Debug)]
pub struct Ac17TraceRevokeTracingKey {
    /// Tracing secret τ
    pub trace_sk: Fr,
...
pub struct Ac17UserIdentityToken {
...
/// AC17 Trace-and-Revoke Secret Key
#[derive(Clone, Debug)]
pub struct Ac17TraceRevokeSecretKey {
    /// Base AC17 secret key
    pub base: SecretKey,
    ...
    pub identity_token: Ac17UserIdentityToken,

    /// Decryption helper for revocation mechanism
    pub revoke_key: G2,
}

impl Ac17TraceRevokeSecretKey {
    /// Get user ID
...
#[derive(Clone, Debug)]
pub struct Ac17RevocationHeader {
...
/// AC17 Trace-and-Revoke Ciphertext
#[derive(Clone, Debug)]
pub struct Ac17TraceRevokeCiphertext {
    /// Base ABE ciphertext
    pub abe_ct: Ciphertext,

    /// Revocation header
    pub revoke_header: Ac17RevocationHeader,
}

/// Full AC17 Trace-and-Revoke Ciphertext with payload
#[derive(Clone, Debug)]
pub struct Ac17TraceRevokeFullCiphertext {
    /// ABE ciphertext with revocation
    pub ct: Ac17TraceRevokeCiphertext,
...
pub fn setup<R: RngCore + CryptoRng>(rng: &mut R) -> (Ac17TraceRevokeMpk, Ac17TraceRevokeMsk, Ac17TraceRevokeTracingKey) {
    ...
    let mpk = Ac17TraceRevokeMpk {
        base: base_mpk,
        trace_vk,
        revoke_base,
    };

    let msk = Ac17TraceRevokeMsk {
        base: base_msk,
        trace_sk,
        revoke_sk,
    };

    let tracing_key = Ac17TraceRevokeTracingKey {
        trace_sk,
        trace_vk,
    };
...
pub fn keygen<R: RngCore + CryptoRng>(
    rng: &mut R,
    mpk: &Ac17TraceRevokeMpk,
    msk: &Ac17TraceRevokeMsk,
    user_id: impl Into<UserId>,
    attributes: &[String],
) -> Result<Ac17TraceRevokeSecretKey, AbeError> {
    ...
    let identity_token = Ac17UserIdentityToken {
        id_hash: h_id,
        revoke_token,
    };
...
pub fn encrypt<R: RngCore + CryptoRng>(
    ...
    plaintext: &[u8],
) -> Result<Ac17TraceRevokeFullCiphertext, AbeError> {
    ...
    let revoke_header = Ac17RevocationHeader {
        revoked_ids,
        revoke_components,
        r_rev,
    };
    ...
    let ct = Ac17TraceRevokeCiphertext {
        abe_ct: base_ct.abe_ct,
        revoke_header,
    };

    Ok(Ac17TraceRevokeFullCiphertext {
        ct,
...
pub fn decrypt(
    mpk: &Ac17TraceRevokeMpk,
    sk: &Ac17TraceRevokeSecretKey,
    ct: &Ac17TraceRevokeFullCiphertext,
    ...
    crate::schemes::ac17::decrypt(&mpk.base, &sk.base, &full_ct)
}

/// Trace a leaked key
pub fn trace_key(
    _mpk: &Ac17TraceRevokeMpk,
    tracing_key: &Ac17TraceRevokeTracingKey,
    leaked_key: &Ac17TraceRevokeSecretKey,
    known_users: &[UserId],
) -> TraceResult {
...
mod tests {
    ...
    #[test]
    fn test_ac17_trace_revoke_setup() {
    ...
    #[test]
    fn test_ac17_trace_revoke_keygen() {
        let mut rng = thread_rng();
    ...
    #[test]
    fn test_ac17_trace_revoke_encrypt_decrypt() {
        let mut rng = thread_rng();
        ...
        let plaintext = b"AC17 Trace-revoke secret";
    ...
    #[test]
    fn test_ac17_revoked_user_cannot_decrypt() {
        let mut rng = thread_rng();
        let (mpk, msk, _tk) = setup(&mut rng);
    ...
    #[test]
    fn test_ac17_encrypt_with_revocation_list() {
        let mut rng = thread_rng();
        let (mpk, msk, _tk) = setup(&mut rng);
